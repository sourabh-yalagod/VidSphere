...
export default function Settings() {
  ...
  const handleChangePassword = async ({ oldPassword, newPassword }) => {
    const response = await axios.patch(`${process.env.BASE_URL}/api/v1/users/change-password`, {
  ...
  const handleChangeAvatar = async ({ avatar }) => {
    ...
    const response = await axios.patch(
      `${process.env.BASE_URL}/api/v1/users/change-avatar`,
      formData,
      {
        headers: {
  ...
  const handleChangeCoverImage = async ({ coverImage }) => {
    ...
    const response = await axios.patch(
      `${process.env.BASE_URL}/api/v1/users/change-coverimage`,
      formdata
    );
    return response?.data;
  };
  ...
  const handleChangeNewToken = async () => {
    const response = await axios.get(`${process.env.BASE_URL}/api/v1/users/generate-newtokens`);
    return response?.data;
  };
